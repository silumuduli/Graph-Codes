...
density.plot2=function(u,v,xtitle="Inflation",name1="Before", name2="After",adj=2){
  ...
  p=p+expand_limits(x = min(value) -0.5*(max(value)-min(value)))+expand_limits(x = max(value) +0.5*(max(value)-min(value)))
  p=p+xlab(xtitle) + ylab("Density")+theme(legend.position="bottom",legend.title = element_blank())+annotate("text", x =c( max(value), max(value)), y = c(0.75*max((density(value)$y)),0.67*max((density(value)$y))), label =c(paste('Mean Difference = ',md),paste('t-test p-value <',max(pval,0.01))),parse = FALSE)
  p
}
